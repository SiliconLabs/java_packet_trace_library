...
public class ExpectConnection {
  ...
  /**
   * This method is functionally the same as expect where 2000ms is used as the
   * timeout and the collect flag is set to false.
   *
   * @param message the message to send.
   * @param regex   the regular expression to look for.
   * @return the matching message, or a concatenation of all incoming messages up
   *         to and including the match if <code>collect</code> is true. Returns
   *         <code>null</code> if nothing matched.
  ...
  /**
   * Sends a message to the device and waits <code>timeout</code> milliseconds to
   * receive an incoming message matching <code>regex</code>. Returns the matching
   * message. If <code>collect</code> is true, concatenates and returns all
   * incoming messages up to and including the match. If <code>regex</code> is
   * null, no matching is attempted. This is useful when collecting all responses
   * to a message without knowing in advance what they are.
   * <p>
   ...
   * <code>IFramer</code> within the <code>IConnection</code> passed at the time
   * this object was created. Messages are concatenated with "\r\n". See
  ...
  /**
   * This is the preferred method of using the expect connection. It is basically
   * the same as {@link #expect(String, String, int, boolean)} with the added
   * functionality of returning an {@link ExpectResponse} object which contains
   * complete information about what happened with the sent message.
   *
   * @param message The message to send to the connection provided when this
   *                {@link #ExpectConnection(IConnection)} was created.
   * @param regex   The regular expression used to match against the response to
   *                the message provided.
   * @param timeout The amount of time to wait before returning either a failed
   ...
   */
  public synchronized ExpectResponse expect(final String message, final String regex, final int timeout) {
    if (LOG)
      System.err.println(Thread.currentThread().hashCode() + ": EXPECT: " + message + " / " + regex);
    // Param verify.
    ...
    // Evaluate response.
    if (LOG)
      System.err.println(Thread.currentThread().hashCode() + ": COLLECTED: " + collectedOutput.toString());
    ExpectResponse response;
    if (pattern == null || pattern.matcher(collectedOutput.toString()).matches()) {
      if (LOG)
        System.err.println(Thread.currentThread().hashCode() + ": MATCHES!!!!!!!!!!");
      response = new ExpectResponse(true, null, currentOutput, collectedOutput.toString());
    } else {
      if (LOG)
        System.err.println(Thread.currentThread().hashCode() + ": NO MATCH... :(");
      response = new ExpectResponse(false, "No match.", null, collectedOutput.toString());
    }
  ...
  /**
   * This method is used by the {@link IConnection} to pass data received back to
   * be processed by the blocking thread which originally called
   * {@link #expect(String, String, int, boolean)}).
   *
   * @param messageBytes The bytes which were received by the {@link IConnection}
   */
