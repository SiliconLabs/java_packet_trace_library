 ...
 * that generated the message, the type of the debug message, the timestamp of
 * the message, and the actual contents of the message, if any, as a byte array.
 *
...
public class DebugMessage {
  ...
  public static enum Version {
    UNKNOWN(-1, -1, -1), V1(1, 1, 1), V2(2, 1, 2), V3(3, 2, 2);
  ...
  public DebugMessage(final long networkTime, final int debugType, final byte[] contents) {
   ...
   * @return byte
   */
  public byte contents(final int i) {
    return contents[i];
  }
  ...
  /**
   * Sets the network time for the debug message, after the time correction was
   * done.
   *
   * @param t
   */
  ...
  /**
   * Constructs a DebugMessage object from the raw message bytes. Assumes that the
   ...
   *
   * Version 1.0 Message: 0 timestamp low byte \ LSB of timestamp = 1/1024 of a
   * second 1 timestamp med byte | 2 timestamp med byte | 3 timestamp high byte /
   * 4 Debug Message Type 5 Debug Message Subtype (EM_DEBUG_MAC_RX) 6 Link Quality
   ...
   *                     received.
   */
  public static DebugMessage makeVersion1(final String originatorId, final byte[] raw, final long pcTime) {
    if (raw.length < 5)
      return null;
   ...
   * length bytes have been removed.
   *
   * Version 2.0 Message: 0 version number \ LSB, direction (0 = node to pc, 1 =
   * pc to node) 1 version number / MSB 2 timestamp byte 0 \ LSB, microsecond tics
   * 3 timestamp byte 1 | 4 timestamp byte 2 | 5 timestamp byte 3 | 6 timestamp
   * byte 4 | 7 timestamp byte 5 / MSB 8 Debug Message Type \ LSB (coresponds to
   * single byte message type. 9 Debug Message Type / MSB 10 sequence number ...
   * Data dependant on the Message Type
   *
   *
   * * Version 3.0 Message: 0 version number \ LSB, direction (0 = node to pc, 1 =
   * pc to node) 1 version number / MSB 2 timestamp byte 0 \ LSB, nanosecond tics
   * 3 timestamp byte 1 | 4 timestamp byte 2 | 5 timestamp byte 3 | 6 timestamp
   * byte 4 | 7 timestamp byte 5 | 8 timestamp byte 6 | 9 timestamp byte 7 / MSB
   * 10 Debug Message Type \ LSB (corresponds to double byte message type. 11
   * Debug Message Type / MSB 12 Flags \ LSB 13 Flags | 14 Flags | 15 Flags / MSB
   * 16 sequence number 17 sequence number ... Data dependent on the Message Type
   *
   * @param originatorId the hostname of the node that generated the message.
   * @param raw          the raw bytes of the message.
   * @param pcTime       the PC system time (in milliseconds) when the message was
   *                     received.
   */
  public static DebugMessage make(final String originatorId, final byte[] raw, final long pcTime) {
    if (raw.length < 2)
      return null;
  ...
  /**
   * Converts an array of bytes to a long. The LSB is raw[startIndex] and the MSB
