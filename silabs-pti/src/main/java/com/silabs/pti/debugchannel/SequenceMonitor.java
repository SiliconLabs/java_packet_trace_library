 ...
 *
 * It can deal with the fact that sequence # can be randomly switching between
 * different byte counts, and will assume that lower bytes follow the sequence.
 *
...
public class SequenceMonitor {
  ...
  /**
   * This is returned by the new sequence, if there is potential for missmatch,
   * because bytecount changed
   */
  ...
  /**
   * This method return value is as follows: a non-negative value means an error
   * and specifies an expected sequence number. a negative value means a success,
   * and can be one of OK_ values above.
   * 
   * @return int
   */
