 ...
 * 
 * and splits it into two output data streams:
 * 1.) abcdef
 * 2.) OPEN_BRACKET123CLOSE_BRACKETOPEN_BRACKET456CLOSE_BRACKET.
 * 
 * So output 1 contains everything that is not within the brackets.
 * And output 2 contains both brackets and what's inside them.
 * 
...
public class BracketSplitter implements ISplitter {
  ...
  private final ICharacterListener[] buckets 
    = new ICharacterListener[bucketCount()];
   ...
   */
  public BracketSplitter(ICharacterMatcher openBracket, 
                         ICharacterMatcher closeBracket) {
  ...
  public void received(byte[] input, int offset, int len) {
    ...
    for ( int i=offset; i<offset+len; i++ ) {
      byte c = input[i];
      ICharacterMatcher expectedBracket
      = ( insideBrackets ? closeBracket : openBracket );
